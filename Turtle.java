...
public class Turtle implements Runnable, ActionListener, MouseListener, MouseMotionListener, KeyListener, ComponentListener, MouseWheelListener
{
    ...
    public static void main(String[] args) {
        ...
        // วาดกระทง (ฐานวงกลม)
        drawCircle(bob, 200, "green");
    
        // วาดชั้นกระทง (วงกลมเล็กซ้อน)
        bob.up();
        bob.left(90);
        bob.forward(50);
        bob.right(90);
        bob.down();
        drawCircle(bob, 150, "yellow");
    ...
    // ฟังก์ชันวาดวงกลม
    public static void drawCircle(Turtle t, int radius, String color) {
        t.penColor(color);
        t.down();
        for (int i = 0; i < 360; i++) {
            t.forward((2 * Math.PI * radius) / 360); // คำนวณความยาวแต่ละส่วน
            t.left(1);
        }
        t.up(); // ยกปากกาเมื่อวาดเสร็จ
    }
    
    // ฟังก์ชันวาดดอกไม้
    public static void drawFlower(Turtle t, int size, String color) {
        t.penColor(color);
        for (int i = 0; i < 12; i++) {
            for (int j = 0; j < 2; j++) {
                t.forward(size);
